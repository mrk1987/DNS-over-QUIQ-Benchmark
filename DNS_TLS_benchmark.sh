...
echo -e "${BLUE}=== DNS DoT Benchmark Suite (Direct Resolver Queries) ===${NC}"
...
benchmark_resolver() {
    ...
    for ((i=0; i<QUERIES; i++)); do
        ...
        (
            ...
            # Use drill with explicit resolver and DoT (-T flag for TLS on port 853)
            # drill queries the specific resolver directly
            timeout 6 drill -T @$host $domain A > /dev/null 2>&1
            result=$?
